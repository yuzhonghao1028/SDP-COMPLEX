...
# load RDS data
seurat.obj.combined <- readRDS(file = "result/SCT.CCA/undefined verify/goat.ON.snRNA_SCT.CCA_undefined verify.rds")
...
ggsave("official figure/celltype/celltype.splitby.group.umap.pdf",width = 12,height = 5)
...
DimPlot(seurat.obj.combined,group.by = "celltype",split.by = "group",cols = seurat.col)+NoAxes()+labs(title = "")
ggsave("result/SCT.CCA/basic processing/celltype.splitby.group.umap.pdf",width = 12,height = 5)

# findallmarkers
DEGs <- FindAllMarkers(seurat.obj.combined,logfc.threshold = 0.25,min.pct = 0.25,verbose = T)
write.table(deg1,file = "result/SCT.CCA/basic processing/celltype_deg.xls",sep = "\t")
...
ggsave(filename = "result/SCT.CCA/basic processing/celltype_marker_dotplot.pdf",width = 6,height = 5.5)
...
ggsave(filename = "result/SCT.CCA/basic processing/stack barplot.pdf",width = 4.5,height = 5)
...
ggsave(filename = "result/SCT.CCA/basic processing/Two-way bar charts_log2fc1.pdf",width = 4,height = 5)
