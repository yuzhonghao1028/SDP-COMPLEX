...
hm_average_selective <- hm_average_selective[,c("MP_injury","MP_control")]
...
## renamed cluster
Idents(MP) <- "reresolution_res.0.2"
MP <- RenameIdents(MP,
                    "1"="1",
                    "2"="others",
                    "3"="others",
...
saveRDS(dge_res, file='results/SCT.CCA/MP & astrocyte processing/zinb_DGE_result_MP_subcluster1.rds') 
## Summarize results
...
openxlsx::write.xlsx(genes.sig.lfc, file='results/SCT.CCA/MP & astrocyte processing/zinb_sig_genes MP_subcluster1 0.25lfc.xlsx') 
...
dge_res <- glmWeightedF(fit, contrast=contrast)
## Save DGE so it doesn't need to be recalculated
saveRDS(dge_res, file='results/SCT.CCA/MP & astrocyte processing/zinb_DGE_result_MP_subcluster2.rds')  
## Summarize results
genes <- topTags(dge_res, n=2000, adjust.method = "BH", sort.by = "PValue", p.value = 0.05)
## Create dataframe with gene name, log2 fold change, and p-value
genes.sig <- data.frame(row.names(genes))
...
genes.sig.lfc <- subset(genes.sig, genes.sig$logFC >= 0.25 | genes.sig$logFC <= -0.25)
## Save dataframe as csv file
openxlsx::write.xlsx(genes.sig.lfc, file='results/SCT.CCA/MP & astrocyte processing/zinb_sig_genes MP_subcluster2 0.25lfc.xlsx') 
...
dge_res <- glmWeightedF(fit, contrast=contrast)
## Save DGE so it doesn't need to be recalculated
saveRDS(dge_res, file='results/SCT.CCA/MP & astrocyte processing/zinb_DGE_result_MP_subcluster3.rds') 
## Summarize results
genes <- topTags(dge_res, n=2000, adjust.method = "BH", sort.by = "PValue", p.value = 0.05)
## Create dataframe with gene name, log2 fold change, and p-value
genes.sig <- data.frame(row.names(genes))
...
genes.sig.lfc <- subset(genes.sig, genes.sig$logFC >= 0.25 | genes.sig$logFC <= -0.25)
## Save dataframe as csv file
openxlsx::write.xlsx(genes.sig.lfc, file='results/SCT.CCA/MP & astrocyte processing/zinb_sig_genes MP_subcluster3 0.25lfc.xlsx') 
...
dge_res <- glmWeightedF(fit, contrast=contrast)
## Save DGE so it doesn't need to be recalculated
saveRDS(dge_res, file='results/SCT.CCA/MP & astrocyte processing/zinb_DGE_result_MP_subcluster4.rds')  
## Summarize results
genes <- topTags(dge_res, n=2000, adjust.method = "BH", sort.by = "PValue", p.value = 0.05)
## Create dataframe with gene name, log2 fold change, and p-value
genes.sig <- data.frame(row.names(genes))
...
genes.sig.lfc <- subset(genes.sig, genes.sig$logFC >= 0.25 | genes.sig$logFC <= -0.25)
## Save dataframe as csv file
openxlsx::write.xlsx(genes.sig.lfc, file='results/SCT.CCA/MP & astrocyte processing/zinb_sig_genes MP_subcluster4 0.25lfc.xlsx')
...
dge_res <- glmWeightedF(fit, contrast=contrast)
## Save DGE so it doesn't need to be recalculated
saveRDS(dge_res, file='results/SCT.CCA/MP & astrocyte processing/zinb_DGE_result_MP_subcluster5.rds') 
## Summarize results
genes <- topTags(dge_res, n=2000, adjust.method = "BH", sort.by = "PValue", p.value = 0.05)
## Create dataframe with gene name, log2 fold change, and p-value
genes.sig <- data.frame(row.names(genes))
