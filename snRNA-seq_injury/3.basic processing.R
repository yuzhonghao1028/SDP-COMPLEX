...
# create folder
dir.create("snRNA_seq_injury/results/SCT.CCA/3.basic processing",recursive = T)
...
ggsave("snRNA_seq_injury/results/SCT.CCA/3.basic processing/celltype.umap.pdf",width = 7.5,height = 5)
DimPlot(seurat.obj.combined,group.by = "celltype",split.by = "group",cols = seurat.col)+NoAxes()+labs(title = "")
ggsave("snRNA_seq_injury/results/SCT.CCA/3.basic processing/celltype.splitby.group.umap.pdf",width = 12,height = 5)

# findallmarkers
DEGs <- FindAllMarkers(seurat.obj.combined,logfc.threshold = 0.25,min.pct = 0.25,verbose = T)
write.xlsx(DEGs,file = "snRNA_seq_injury/results/SCT.CCA/3.basic processing/celltype_deg.xlsx",sep = "\t")

# dotplot for marker genes
DotPlot(seurat.obj.combined,features = c("MOBP","MBP","P2RY12","CD36","GFAP","ALDH1L1","FAP","COL3A1",
                                         "ERG","FLT1","PDGFRA","PCDH15","PDGFRB","ACTA2"), cols = c("grey", "#ff70b5"))+RotatedAxis()+ggplot2:::coord_flip()+ylab("")+xlab("")
ggsave(filename = "snRNA_seq_injury/results/SCT.CCA/3.basic processing/celltype_marker_dotplot.pdf",width = 6,height = 5.5)
...
ggsave(filename = "snRNA_seq_injury/results/SCT.CCA/3.basic processing/stack barplot.pdf",width = 4.5,height = 5)
...
for (i in celltype_name){
  ...
  saveRDS(sce.zinb, file=paste0('snRNA_seq_injury/results/SCT.CCA/3.basic processing/zinb_weights_',cluster_choose,'.rds')) 
  ...
  saveRDS(dge_res, file=paste0('snRNA_seq_injury/results/SCT.CCA/3.basic processing/zinb_DGE_result_',cluster_choose,'.rds'))  
  ## Summarize results
  ...
  openxlsx::write.xlsx(genes.sig.lfc, file=paste0('snRNA_seq_injury/results/SCT.CCA/3.basic processing/zinb_sig_genes_',cluster_choose,'-0.25lfc.xlsx')) 
}

# DEGs two-way bar charts
df <- read.csv(file = "snRNA_seq_injury/results/SCT.CCA/3.basic processing/DEG number_log2fc1.csv")
df <- melt(df) 
ggplot(df, aes(
...
ggsave(filename = "snRNA_seq_injury/results/SCT.CCA/3.basic processing/Two-way bar charts_log2fc1.pdf",width = 4,height = 5)
